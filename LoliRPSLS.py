...
def determine_winner(move1, move2):
    if move1 == move2:
        return None, "It's a tie! No HP lost."
    outcomes = {
    ...
    if (move1, move2) in outcomes:
        return 1, outcomes[(move1, move2)]
    elif (move2, move1) in outcomes:
        return 2, outcomes[(move2, move1)]
    else:
...
def player_move(player_choice):
    ...
    winner, outcome = determine_winner(move1, move2)
    if winner == 1:
        loli2.hp -= 1
        round_result += f"\n{outcome}. {loli2.name} loses 1 HP! Remaining HP: {loli2.hp}"
    elif winner == 2:
        loli1.hp -= 1
        round_result += f"\n{outcome}. {loli1.name} loses 1 HP! Remaining HP: {loli1.hp}"
    else:
